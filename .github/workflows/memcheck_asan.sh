...
if [[ "$GITHUB_EVENT_NAME" == "pull_request" ]]; then
    ...
    echo "meson -Db_sanitize=\"address,undefined\" -Dloaders=\"all\" -Dsavers=\"all\" -Dtests=\"unit\" -Dbindings=\"capi\" . build"
    pwd
    cd ${GITHUB_WORKSPACE}/build/test/unit
    ...
    if [[ $PAYLOAD_MEMCHECK == *"runtime error:"* || $PAYLOAD_MEMCHECK == *"ERROR: AddressSanitizer:"* ]]; then
        ...
        OUTPUT+=$'\n`meson -Db_sanitize="address,undefined" -Dloaders="all" -Dsavers="all" -Dtests="unit" -Dbindings="capi" . build`\n'
        OUTPUT+=$'\n```\n'
