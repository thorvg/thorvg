...
jerry_context_data (const jerry_context_data_manager_t *manager_p)
{
  ...
  item_p = (jerry_context_data_header_t *) jmem_heap_alloc_block (sizeof (jerry_context_data_header_t) + manager_p->bytes_needed);
...
jerry_object_type (const jerry_value_t value) /**< input value to check */
{
  ...
  switch (ecma_get_object_type (obj_p))
  {
    ...
    case ECMA_OBJECT_TYPE_BUILT_IN_CLASS:
    {
      ...
      return (jerry_object_type_t) jerry_class_object_type[ext_obj_p->u.cls.type];
    }
                 ...
                 const jerry_value_t rhs) /**< second operand */
{
  ...
  switch (operation)
  {
    ...
    {
      return jerry_return (do_number_arithmetic ((number_arithmetic_op) ((int) operation - ECMA_NUMBER_ARITHMETIC_OP_API_OFFSET), lhs, rhs));
    }
    default:
    {
                                     ...
                                     void *user_data_p) /**< pointer to user data */
{
  ...
  while (iter_cp != JMEM_CP_NULL)
  {
    ...
    {
      native_pointer_p = ecma_get_native_pointer_value (iter_p, (jerry_object_native_info_t *) native_info_p);
                                ...
                                const jerry_object_native_info_t *native_info_p) /**< object's native type info */
{
  if (ecma_is_value_object (object))
  {
    ...
    return ecma_delete_native_pointer_property (object_p, (jerry_object_native_info_t *) native_info_p);
  }
...
jerry_promise_state (const jerry_value_t promise) /**< promise object to get the state from */
{
  ...
  return (flags ? (jerry_promise_state_t) flags : JERRY_PROMISE_STATE_REJECTED);
...
{
#if JERRY_BUILTIN_TYPEDARRAY
  ecma_builtin_id_t prototype_id = (ecma_builtin_id_t) 0;
  ecma_typedarray_type_t id = (ecma_typedarray_type_t) 0;
                          ...
                          bool *is_key_value_p) /**< [out] is key-value structure */
{
  ...
  *is_key_value_p = false;
      ...
      || ext_obj_p->u.cls.type == ECMA_OBJECT_CLASS_SET_ITERATOR)
  {
    ...
    entry_size = ecma_op_container_entry_size ((lit_magic_string_id_t) map_object_p->u.cls.u2.container_id);
  ...
  else if (jerry_container_type (value) != JERRY_CONTAINER_TYPE_INVALID)
  {
    ...
    entry_count = ECMA_CONTAINER_ENTRY_COUNT (container_p);
    entry_size = ecma_op_container_entry_size ((lit_magic_string_id_t) ext_obj_p->u.cls.u2.container_id);
                    ...
                    uint32_t arguments_number) /**< number of arguments */
{
  ...
  ecma_extended_object_t *container_object_p = ecma_op_container_get_object (container, (lit_magic_string_id_t) type);
  ...
  switch (operation)
  {
    case JERRY_CONTAINER_OP_ADD:
    {
      ...
      result = ecma_op_container_set (container_object_p, arguments[0], arguments[0], (lit_magic_string_id_t) type);
      break;
    }
    case JERRY_CONTAINER_OP_GET:
    {
      ...
      result = ecma_op_container_get (container_object_p, arguments[0], (lit_magic_string_id_t) type);
      break;
    }
    case JERRY_CONTAINER_OP_SET:
    {
      ...
      result = ecma_op_container_set (container_object_p, arguments[0], arguments[1], (lit_magic_string_id_t) type);
      break;
    }
    case JERRY_CONTAINER_OP_HAS:
    {
      result = ecma_op_container_has (container_object_p, arguments[0], (lit_magic_string_id_t) type);
      break;
    }
    case JERRY_CONTAINER_OP_DELETE:
    {
      ...
      {
        result = ecma_op_container_delete_weak (container_object_p, arguments[0], (lit_magic_string_id_t) type);
        break;
      }
      result = ecma_op_container_delete (container_object_p, arguments[0], (lit_magic_string_id_t) type);
      break;
    }
    case JERRY_CONTAINER_OP_SIZE:
    {
