...
void Vertex::insertAbove(Edge *e) {
  ...
  for (above_next = this->edge_above.head; above_next;
  ...
  LinkedList<Edge>::Insert<&Edge::above_prev, &Edge::above_next>(
      e, above_prev, above_next, &this->edge_above.head,
...
void Vertex::insertBelow(Edge *e) {
  ...
  // find insertion point
  for (below_next = this->edge_below.head; below_next;
  ...
  LinkedList<Edge>::Insert<&Edge::below_prev, &Edge::below_next>(
      e, below_prev, below_next, &this->edge_below.head,
...
bool Edge::intersect(Edge *other, Point *point) {
  ...
  point->x =
      std::round(static_cast<float>(top->point.x - s_number * le_b * scale));
  point->y =
      std::round(static_cast<float>(top->point.y + s_number * le_a * scale));
...
void ActiveEdgeList::insert(Edge *e, Edge *prev, Edge *next) {
  LinkedList<Edge>::Insert<&Edge::left, &Edge::right>(e, prev, next, &head,
                                                      &tail);
}
...
void ActiveEdgeList::append(Edge *e) { insert(e, tail, nullptr); }
...
Polygon *Polygon::addEdge(Edge *e, Side side, ObjectHeap *heap) {
  ...
  if (!this->tail) {
    this->head = this->tail =
        heap->Allocate<MonotonePolygon>(e, side, this->winding);
    this->count += 2;
...
void MonotonePolygon::addEdge(Edge *edge) {
  if (this->side == Side::kRight) {
    LinkedList<Edge>::Insert<&Edge::right_poly_prev, &Edge::right_poly_next>(
        edge, this->last, nullptr, &this->first, &this->last);
  } else {
    LinkedList<Edge>::Insert<&Edge::left_poly_prev, &Edge::left_poly_next>(
        edge, last, nullptr, &this->first, &this->last);
  }
}
...
Tessellator::Tessellator()
    : pHeap(new detail::ObjectHeap), outlines(), pMesh(new detail::VertexList),
      pPolygon(), resPoints(), resIndices() {}
...
void Tessellator::tessMesh() {
  ...
  for (auto v = pMesh->head; v; v = v->next) {
    ...
    if (v->edge_above.head) {
      ...
      if (left_poly) {
        left_poly = left_poly->addEdge(v->edge_above.head, detail::Side::kRight,
                                       pHeap.get());
      }

      if (right_poly) {
        right_poly = right_poly->addEdge(v->edge_above.tail,
                                         detail::Side::kLeft, pHeap.get());
      }
      ...
      if (!v->edge_above.head) {
        ...
        if (left_poly && right_poly) {
          ...
          if (left_poly == right_poly) {
             ...
             */
            if (left_poly->tail &&
                left_poly->tail->side == detail::Side::kLeft) {
              left_poly =
                  this->makePoly(left_poly->lastVertex(), left_poly->winding);

              left_enclosing->right_poly = left_poly;
            } else {
              right_poly =
                  this->makePoly(right_poly->lastVertex(), right_poly->winding);
            }
          }

          // need to link this vertex to above polygon
          auto join =
              pHeap->Allocate<detail::Edge>(left_poly->lastVertex(), v, 1);

          left_poly =
              left_poly->addEdge(join, detail::Side::kRight, pHeap.get());
          right_poly =
              right_poly->addEdge(join, detail::Side::kLeft, pHeap.get());
        }
      }
...
bool Tessellator::checkIntersection(detail::Edge *left, detail::Edge *right,
                                    detail::ActiveEdgeList *ael,
...
bool Tessellator::splitEdge(detail::Edge *edge, detail::Vertex *v,
                            detail::ActiveEdgeList *ael,
                            detail::Vertex **current) {
...
bool Tessellator::intersectPairEdge(detail::Edge *left, detail::Edge *right,
                                    detail::ActiveEdgeList *ael,
                                    detail::Vertex **current) {
  ...
  if (detail::VertexCompare::Compare(right->bottom->point,
                                     left->bottom->point)) {
